...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))
# sys.path.append('/home/yuqun/research/feed_forward/Mask2Former')

from odin.modeling.backproject.backproject_2d import backprojector
from odin.utils.util_3d import plot_3d
...
count = 0
for scene_index, scene in enumerate(tqdm(sorted(os.listdir(FRAME_DIR)))):
    if scene_index < 22:
        continue
    ...
    depths = torch.from_numpy(depths / 1000.0).cuda().float() 
    ...
    labels_2d, mask_segments_2d, valids_2d  = get_2d_labels(images, coordinates, labels, segments, depths, poses)
