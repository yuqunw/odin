...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets, radius, meanActive):
        ...
        while True:
            ...
            nActive = pointgroup_ops_cuda.ballquery_batch_p(coords, batch_idxs, batch_offsets, idx, start_len, n, meanActive, radius)
...
class Clustering:
    ...
    def cluster(self, vertices, scores):
        ...
        # scatter
        proposals_pred = torch.zeros((proposals_offset.shape[0] - 1, vertices.shape[0]),
                                     dtype=torch.int)  # (nProposal, N), int, cuda
...
class BFSCluster(Function):
    ...
    def forward(ctx, semantic_label, ball_query_idxs, start_len, threshold):
        ...
        pointgroup_ops_cuda.bfs_cluster(semantic_label, ball_query_idxs, start_len, cluster_idxs, cluster_offsets, N, threshold)
